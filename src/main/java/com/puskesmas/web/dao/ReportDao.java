...
import com.puskesmas.web.model.DashboardStats;
import com.puskesmas.web.model.MonthlySummary;
...
public class ReportDao {
    ...
    public DashboardStats getDashboardStats() throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            stats.setLowStock(count(conn, "SELECT COUNT(*) FROM obat WHERE stok_saat_ini <= stok_minimum"));
        }
        return stats;
    }
    ...
    public List<Obat> findLowStock(int limit) throws SQLException {
        String sql = "SELECT id_obat, kode_obat, nama_obat, kategori, satuan, stok_saat_ini, stok_minimum FROM obat WHERE stok_saat_ini <= stok_minimum ORDER BY stok_saat_ini ASC LIMIT ?";
    ...
    public List<MonthlySummary> getMonthlySummary() throws SQLException {
        ...
        String masukSql = "SELECT to_char(date_trunc('month', tanggal_masuk), 'YYYY-MM') AS bulan, SUM(jumlah) AS total FROM obat_masuk GROUP BY 1 ORDER BY 1";
        String keluarSql = "SELECT to_char(date_trunc('month', tanggal_keluar), 'YYYY-MM') AS bulan, SUM(jumlah) AS total FROM obat_keluar GROUP BY 1 ORDER BY 1";
